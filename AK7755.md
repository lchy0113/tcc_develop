...
static int ak7755_probe(struct snd_soc_codec *codec)
	...
	|	|	|	/* call apply_register with KDIWIN_AUDIO_PATH_NORMAL)
	|	|	+->  static void apply_register(struct snd_soc_codec *codec, KDIWIN_AUDIO_PATH audio_path, int mode)
	|	|	|	|	/**
	|	|	|	|	  * write pram, cram data 
	|	|	|	|	  */
	|	|	|	+->	static int fw_download(struct snd_soc_codec *codec, unsigned char* buffer, int size)
	|	+-> ak7755_interface.audio_path = (KDIWIN_AUDIO_PATH)KDIWIN_AUDIO_PATH_NORMAL;
	|		/* set AUDIO PATH to KDIWIN_AUDIO_PATH_NORMAL */
	(done)
```
...
(call ioctl systemcall)
	...
	|	  * upper layer로 부터 전달 받은 데이터; value(KDIWIN_AUDIO_PATH)을 
	| 	  * ak7755 에 적용 시킨다. 
	|	  */
	|
	...
	|	/** AK7755 에 적용된 현재 AUDIO_PATH 값 value;(KDIWIN_AUDIO_PATH)을 upper layer에게 전달한다. */
	|
	+-> IOCTL_SET_BACK_OUT
	|	/** DOOR_CALL시, BACK_CALL을 출력한다.  (KDIWIN_AUDIO_PATH)
	|	  * AK7755 에서 사용하지 않음. 
	|	  */
	|
	+-> IOCTL_SET_DAMP
