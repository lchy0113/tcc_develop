...
### 1) **타이머1**: 트랜시버의 전송제어(TxEN)신호 타이밍 지연
...
####  2.1) 상위 영역에서 세팅하는 시리얼 장치 설정 값에 따라서 전송되는 bit 의 count를 계산합니다.  
   ex. 비트 수(8bit) + 정지 비트(1bit) + 패리티 비트(1bit)  =  transfer_bits_count(10)  
####  2.2) 문자 한 개를 보내는 데 필요한 시간 계산  
...
####  3.1) 타이머1은 정의된 Transmitter Timing 만큼 지연 후, 전송제어(TxEN)신호를 활성화 합니다.
####  3.2) 정의된 시간(send_character_time)의 타이머2 핸들러를 통해 전송 버퍼 상태를 체크합니다. 
####  3.3) 전송 버퍼 상태 확인(empty) 후, 타이머1은 정의된 Receiver Timing 만큼 지연 후, 전송제어(TxEN)신호를 비활성화 합니다.
