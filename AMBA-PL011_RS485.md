...
## 2. 기술 적용 영역
 ...
 본 문서에서 설명된 기술이 적용된 영역은 아래 그림과 같습니다.  

 ![](images/AMBA-PL011_RS485_07.png)
...
## 3. 종래기술의 문제점
 ...
 ![](images/AMBA-PL011_RS485_05.png)
   ...
   - 데이터 전송 요청 시, 데이터 송신을 담당하는 모듈은 '전송제어(TxEN)신호 활성화, 1ms 지연, 데이터 전송, 1ms 지연, 전송제어(TxEN)신호 비활성화' 과정을 진행합니다.  
   - 전송제어(TxEN) 신호 제어 타이밍을 위해서 **데이터 송신 버퍼 확인 또는 인터럽트 과정이 필요**한데, 종래기술에서의 **문제점은 전송제어(TxEN) 신호를 1ms 지연 함수**로만 타이밍을 맞추고 있습니다.   
...
## 4. 기술 설명
 ...
 전송제어(TxEN)신호 타이밍을 효율적으로 관리하기 위해 다음과 같은 기능을 추가합니다.
   ...
   - **타이머1**,**타이머2**를 사용하여 트랜시버의 리셋 시간을 지연
     * **타이머1**은 트랜시버의 전송제어(TxEN)신호 타이밍을 지연시킵니다.
	 * **타이머2**은 트랜시버 버퍼 상태를 체크합니다. 


### 4.1) **타이머1**: 트랜시버의 전송제어(TxEN)신호 타이밍 지연
...
### 4.2) **타이머2**: 트랜시버 버퍼 상태 체크 타이밍 계산

  트랜시버 버퍼가 비었다는 것을 알려주는 인터럽트가 없는 경우, 폴링 방식으로 트랜시버 버퍼 상태를 체크해야 합니다.   
  전송 데이터 세팅 값에 따라서 트랜시버 버퍼 상태 체크 타이밍이 달라지므로 아래와 같은 방법으로 적절한 타이밍을 계산합니다.  

 ![](images/AMBA-PL011_RS485_02.png) 

 > 시리얼 통신 과정 중, '터미널 설정 변경' 단계에서 전달 받은 값을 활용합니다.

####  4.2.1) 상위 영역에서 세팅하는 시리얼 장치 설정 값에 따라서 전송되는 bit 의 count를 계산합니다.  
   ex. 비트 수(8bit) + 정지 비트(1bit) + 패리티 비트(1bit)  =  transfer_bits_count(10)  
####  4.2.2) 문자 한 개를 보내는 데 필요한 시간 계산  
   ex. transfer_bits_count(10) * nanosecond per second(10^10) / baud rate(9600)  =  send_character_time(10416666)(ns)  


### 4.3) **데이터 전송**: 동기식 전송제어(TxEN)신호 제어

  ![](images/AMBA-PL011_RS485_03.png)   
 > 시리얼 통신 과정 중, '데이터 송신' 단계에서 추가한 기능이 동작됩니다.
####  4.3.1) 타이머1은 정의된 Transmitter Timing 만큼 지연 후, 전송제어(TxEN)신호를 활성화 합니다.
####  4.3.2) 정의된 시간(send_character_time)의 타이머2 핸들러를 통해 전송 버퍼 상태를 체크합니다. 
####  4.3.3) 전송 버퍼 상태 확인(empty) 후, 타이머1은 정의된 Receiver Timing 만큼 지연 후, 전송제어(TxEN)신호를 비활성화 합니다.
...
> 본 기술 적용 전/후 데이터 전송 과정 비교  
  
 ![](images/AMBA-PL011_RS485_08.png)
  
 - RS485 설정 구성을 위한 함수가 추가되어 RS485의 전송 지연, 시점 등을 설정할 수 있습니다.  
 - RS485 전송 처리를 최적화 하여 효율성이 향상되었습니다.   
  (예를 들어, RS485 전송 처리에서 전송제어(TxEN)신호를 비활성화 하는 시점을 조정하여 효율성을 향상시켰습니다.)  
  
### 5.2 기대 효과

 RS485 통신에서 전송제어(TxEN) 신호 타이밍을 전송 데이터에 동기되어 제어하면 다음과 같은 효과를 기대할 수 있습니다.  
  
 - 전송 효율 향상  
	데이터 전송 시작과 종료 시점을 정확하게 제어 할 수 있습니다.   
	이로 인해, 전송 효율이 향상되고, 데이터 손실이 줄어듭니다.   
	전송되는 데이터 length와 동기되어 전송제어가 가능합니다.  
  
 - 데이터 충돌 방지  
	 RS485는 다중 통신 노드가 공통 데이터 라인을 사용하여 통신하는 방식 입니다. 따라서 데이터 충돌이 발생할 수 있습니다.   
	 전송제어(TxEN) 신호를 전송 데이터에 동기되어 제어하면, 데이터 충돌이 발생할 가능성을 줄일 수 있습니다.  
  
 - 전송 안정성 향상  
	 전송제어(TxEN) 신호를 효율적으로 관리하면 전송 데이터의 신호 대 잡음비(SNR)를 높일 수 있습니다. 이로 인해 전송 안정성이 향상됩니다.  
  
 - 장치 오작동 방지  
	전송제어 타이밍 관리를 통해 시스템 자원을 효율적으로 관리 및 운용할 수 있습니다.  
...
---------------------------------
 ...
 - Legacy Code :
  상위 영역로부터 RS-485 데이터 전송 호출을 받으면 아래 순서로 동작합니다.
    * 전송제어(TxEN)신호 활성화
	* 지연 1ms 
	* 데이터 전송
	* 지연 1ms 
